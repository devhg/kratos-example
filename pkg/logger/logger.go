...
func NewJSONLogger(opts ...Option) (*zap.Logger, error) {
	...
	info := zapcore.Lock(zapcore.AddSync(opt.infoFile)) // lock for concurrent safe
	warn := zapcore.Lock(zapcore.AddSync(opt.warnFile)) // lock for concurrent safe

	core := zapcore.NewTee(
		zapcore.NewCore(jsonEncoder,
			zapcore.NewMultiWriteSyncer(info, zapcore.AddSync(os.Stdout)),
			lowPriority,
		),
		zapcore.NewCore(jsonEncoder,
			zapcore.NewMultiWriteSyncer(warn, zapcore.AddSync(os.Stdout)),
			highPriority,
		),
	)
